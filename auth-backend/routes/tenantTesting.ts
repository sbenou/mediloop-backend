...
router.post("/api/test/concurrent-registrations", async (ctx) => {
  ...
  try {
    ...
    console.log(`🚀 Starting concurrent registration test with ${userCount} users`);
    ...
    for (let i = 0; i < userCount; i++) {
      ...
      const registrationPromise = registrationService.registerUser(
        email, 
        password, 
        fullName, 
        role
    ...
    console.log(`✅ Concurrent test completed: ${successful} successful, ${failed} failed in ${totalDuration}ms`);
    ...
    ctx.response.body = {
      ...
      detailedResults: results.map(r => r.status === 'fulfilled' ? r.value : { error: r.reason }),
      timestamp: new Date().toISOString()
    };
...
router.post("/api/test/tenant-resolution", async (ctx) => {
  ...
  try {
    ...
    console.log(`🚀 Testing tenant resolution speed with ${iterations} iterations`);
    
    // Get list of existing tenants
    const tenantsResult = await postgresService.query('SELECT id, domain, schema FROM tenants LIMIT 10');
    ...
    const endTime = performance.now();
    const totalDuration = endTime - startTime;
    const averageDuration = results.reduce((sum, r) => sum + r.durationMs, 0) / results.length;
    
    console.log(`✅ Tenant resolution test completed in ${totalDuration}ms, avg: ${averageDuration}ms per lookup`);
    ...
    ctx.response.body = {
      ...
      performanceBreakdown: {
        ...
        median: results.sort((a, b) => a.durationMs - b.durationMs)[Math.floor(results.length / 2)].durationMs
      },
      timestamp: new Date().toISOString()
    };
...
router.post("/api/test/schema-creation", async (ctx) => {
  ...
  try {
    ...
    console.log(`🚀 Testing schema creation performance with ${testCount} schemas`);
    ...
    for (let i = 0; i < testCount; i++) {
      ...
      try {
        // Create test schema
        await postgresService.query(`CREATE SCHEMA IF NOT EXISTS ${testSchemaName}`);
        ...
        await postgresService.query(`DROP SCHEMA IF EXISTS ${testSchemaName} CASCADE`);
    ...
    console.log(`✅ Schema creation test completed: ${successful}/${testCount} successful in ${totalDuration}ms`);
...
router.post("/api/test/connection-pool", async (ctx) => {
  ...
  try {
    ...
    console.log(`🚀 Testing connection pool with ${concurrentQueries} concurrent queries`);
    
    const startTime = performance.now();
    const promises = [];
    
    for (let i = 0; i < concurrentQueries; i++) {
      const queryPromise = postgresService.query(
    ...
    console.log(`✅ Connection pool test completed: ${successful}/${concurrentQueries} successful, ${uniqueConnections} unique connections used`);
    ...
    ctx.response.body = {
      ...
      testResults: {
        ...
        connectionReuseRatio: (concurrentQueries - uniqueConnections) / concurrentQueries
      },
      timestamp: new Date().toISOString()
    };
