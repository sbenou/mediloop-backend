...
luxtrustRoutes.post('/verify-id', async (c) => {
  try {
    ...
    // Store in KV
    await kvStore.set(['luxtrust_id_verification', sessionId], verificationResponse, { expireIn: 3600000 })
...
luxtrustRoutes.post('/certification/upload', async (c) => {
  try {
    ...
    // Store in KV
    await kvStore.set(['certification_upload', sessionId], certificationResponse, { expireIn: 3600000 })
...
luxtrustRoutes.post('/certification/verify', async (c) => {
  try {
    ...
    // Store in KV
    await kvStore.set(['certification_verification', sessionId], verificationResponse, { expireIn: 3600000 })
