/**
 * Find repo-root `.env.test` whether tests are run from repo root or `backend/`.
 * `dotenv` `envPath: ".env.test"` only checks Deno.cwd(), which breaks `cd backend && deno task test`.
 */
