...
export class TenantManager {
  constructor(private client: PostgresClient, private schemaManager: SchemaManager) {}
  ...
  async updateTenantName(userId: string, workplaceName?: string, pharmacyName?: string): Promise<boolean> {
    try {
      ...
      const { role, full_name } = userResult.rows[0]
  ...
  async deleteTenant(tenantId: string): Promise<boolean> {
    try {
      ...
      const schemaName = tenantResult.rows[0].schema
