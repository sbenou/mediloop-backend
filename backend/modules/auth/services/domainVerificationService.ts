...
export class DomainVerificationService {
  ...
  async initiateDomainVerification(
    ...
    try {
      ...
      const verificationData = result.rows[0]
        .result as DomainVerificationResult;

      // Store verification token in Deno KV for quick access
      const kv = await Deno.openKv();
      await kv.set(
        [`domain_verification`, verificationData.verification_id],
        {
          domain,
  ...
  async verifyDomainOwnership(
    ...
    try {
      ...
      const { domain, token } = kvEntry.value as {
        domain: string;
        token: string;
        tenantId: string;
      };
      ...
      console.log(
        `Domain verification ${verificationResult.success ? "succeeded" : "failed"} for ${domain}`,
      );
  ...
  private async checkDNSVerification(
    ...
    try {
      ...
      console.log(
        `DNS verification for ${txtRecord}: ${simulatedSuccess ? "SUCCESS" : "FAILED"}`,
      );
