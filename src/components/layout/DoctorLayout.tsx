...
const DoctorLayout = ({ children }: DoctorLayoutProps) => {
  ...
  useEffect(() => {
    ...
    console.log("DoctorLayout component mounted");
    ...
    return () => {
      console.log("DoctorLayout component unmounted");
  ...
  // If not authenticated or not a doctor, show minimal loading until redirect happens
  if (!isAuthenticated || (profile && profile.role !== "doctor")) {
    return (
      <div className="flex h-screen items-center justify-center">
        <div className="flex flex-col items-center gap-2">
