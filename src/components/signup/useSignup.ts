...
import { getAuthToggleConfig } from "@/auth-v2/config/authToggle";
import { usePasswordSignupV2 } from "@/auth-v2/hooks/usePasswordSignupV2";
...
export const useSignup = () => {
  ...
  // V2 signup hook
  const v2Signup = usePasswordSignupV2();
  ...
  const handleSignup = async (
    ...
    try {
      ...
      const authConfig = getAuthToggleConfig();
      console.log(
        `Signup: Using ${authConfig.useNewAuthService ? "V2" : "legacy"} auth system`,
      );

      if (authConfig.useNewAuthService) {
        // Use V2 auth system
        console.log("Using V2 signup system");
        const result = await v2Signup.signup(email, password, name, role);

        if (result.success) {
          // V2 signup successful
          if (
            (role === "pharmacist" || role === "doctor") &&
            onRegistrationComplete &&
            result.user
          ) {
            onRegistrationComplete(result.user.id, role);
          } else {
            navigate("/");
          }
        }
      ...
      // Legacy Supabase auth system below
        ...
        try {
          ...
          if (signInError) {
            ...
            toast({
              ...
              description:
                "The email is already registered, but the password is incorrect.",
              variant: "destructive",
            });
            return;
          }
      ...
      try {
        ...
        if (isEmailConfirmationSent) {
          ...
          toast({
            ...
            description:
              "Please check your email for a verification link to complete signup.",
            duration: 6000,
          });
          ...
          if (process.env.NODE_ENV === "development") {
            console.log(
              "Development environment: Allowing login without email verification",
            );
            ...
            setTimeout(async () => {
              try {
                ...
                if (signInData.user) {
                  console.log(
                    "Development: signed in without email verification",
                  );

                  toast({
                    title: "Development Mode",
                    description:
                      "Signed in without email verification (development only).",
                    duration: 4000,
                  });
                ...
                console.error(
                  "Development sign in attempt failed:",
                  devSignInError,
                );
              }
            }, 1500);
        ...
        if (
          ...
          console.log(
            "Email sending error detected - handling specially in development",
          );
          ...
          if (process.env.NODE_ENV === "development") {
            ...
            try {
              ...
              if (insertError) {
                console.error(
                  "Error creating development profile:",
                  insertError,
                );
                throw insertError;
              }
              ...
              console.log(
                "Created development profile without email verification",
              );

              toast({
                title: "Development Account Created",
                description:
                  "Created an account while bypassing email verification (development only).",
                duration: 4000,
              });
              ...
              console.error(
                "Development profile creation failed:",
                devProfileError,
              );
