        ...
        <div className="space-y-6">
          {availableRoutePermissions.map((routePermission) => (
            <div key={routePermission.route} className="border rounded-lg p-4">
              <h3 className="text-lg font-semibold mb-2 text-left">{routePermission.route}</h3>
              <div className="space-y-2">
                {routePermission.permissions.map((permission) => (
                  <div key={permission.id} className="bg-accent/30 p-3 rounded-md">
                    <div className="font-medium text-left">{permission.name}</div>
                    <div className="text-sm text-muted-foreground text-left">{permission.description}</div>
                    <div className="text-xs text-muted-foreground mt-1 text-left">ID: {permission.id}</div>
                  </div>
                ))}
              </div>
            </div>
